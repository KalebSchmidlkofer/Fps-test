...
@export var character_speed: float = 1
...
func normal_mode(delta):
	...
	if Input.is_action_pressed("slide"):
		var slideDirection=start_slide(delta, direction)
	if Input.is_action_just_released('slide'):
		if is_sliding:
			end_slide()

	if is_sliding:
		print('is_sliding and')
...
func start_slide(delta, direction) -> Vector3:
	is_sliding = true
	player.scale.y = slide_size
	uncrouchray.scale.y = slide_size

	if direction:
		velocity.x = direction.x * slide_speed*3500 * delta
		velocity.z = direction.z * slide_speed*3500 * delta
	else:
		velocity.x = move_toward(velocity.x, 0, slide_speed*3500)
		velocity.z = move_toward(velocity.z, 0, slide_speed*3500)
	return direction
